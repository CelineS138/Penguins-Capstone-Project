...
master_energy <- master_energy |> 
  group_by(Player, Year) |> 
  mutate(total_zscore = sum(normalized_loadings[1, 1] * HIT, normalized_loadings[2, 1] * BLK, normalized_loadings[3, 1] * PIM)) |> 
  ungroup() |> 
  group_by(Player) |> 
  mutate(mean_zscore = mean(total_zscore, na.rm = TRUE))
...
master_energy <- master_energy |> 
  ...
  mutate(qtr1 = quantile(total_zscore, 0.25, na.rm = TRUE),
         qtr2 = quantile(total_zscore, 0.5, na.rm = TRUE),
         qtr3 = quantile(total_zscore, 0.75, na.rm = TRUE),
         skill = ifelse(total_zscore <= qtr1, "Worst", ifelse(total_zscore >= qtr3, "Great", ifelse(total_zscore < qtr3 & total_zscore >= qtr2, "Good", "Bad")))
         )
...
master_energy2 |> 
  ...
  scale_y_discrete(limits=c('Good Consistent', 'Ok Consistent', 'Bad Consistent')) +
  scale_fill_manual(values = c("Bad Consistent" = "purple1", "Ok Consistent" = "orange", "Good Consistent" = "blue")) +
  labs(
    x = "Number of Players",
...
master_energy2 |> 
  ...
  # scale_y_discrete(limits=c('Good Consistent', 'Ok Consistent', 'Bad Consistent')) +
  scale_fill_discrete(breaks=c('Worst', 'Bad', 'Good', 'Great')) +
  labs(
    x = "Number of Players",
    y = "<-- Better 'Energy' Ability",
    caption = "Data Courtesy of Hockey-Reference"
