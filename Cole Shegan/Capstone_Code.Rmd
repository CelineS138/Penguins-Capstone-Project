...
```{r}
feat <- complete_df |> select(GP:TOI, BLK:`FO%`, CF:DPS) |> select(-`TOI/60`, -`TOI(EV)`, -Shift, -`TOI (EV)`, -`TOI (PP)`, -`TOI (SH)`) |> na.omit()
...
master_playmaker |> 
  ...
  geom_beeswarm(aes(color = Groupings), alpha = 0.2) +
  geom_hline(yintercept = 0.5, color = "red", linetype = "dashed", size = 2) +
  geom_hline(yintercept = -0.5, color = "red", linetype = "dashed", size = 2) +
  geom_label_repel(size = 2, box.padding = .05) +
